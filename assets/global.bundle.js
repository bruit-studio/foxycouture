...
const resizeObserver = new ResizeObserver(entries => {
  entries.forEach(entry => {
    ...
    if (matches) {
      ...
      const sum = Object.values(heightWatchers[group]).reduce((a, b) => a + b, 0);
      document.documentElement.style.setProperty(`--${group}-height`, `${sum}px`);
    }
  });
});
const heightElements = document.querySelectorAll('.shopify-section-group-header-group, .shopify-section-group-hero-group');
heightElements.forEach(element => {
  resizeObserver.observe(element);
});
console.log('Exhibit theme (1.0.9) by SWITCH | Make the switch: https://switchthemes.co');
